...
class PositionalEncoding(nn.Module):
    """Sinusoidal positional encoding for inputs of shape (T, N, C)."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: Tensor of shape (T, N, C)
        Returns:
            Tensor of shape (T, N, C)
        """
        T = x.size(0)
        if T > self.pe.size(0):
            new_pe = self._build_pe(T, self.d_model).to(x.device)
            self.pe = new_pe
...
class TransformerEncoder(nn.Module):
    """Transformer encoder wrapper for time-first inputs of shape (T, N, C)."""
    ...
    def __init__(
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            activation="relu",
            batch_first=False,   # important: input is (T, N, C)
            norm_first=True,     # usually trains a bit more stably
        )
    ...
    def forward(
        ...
        Args:
            ...
            input_lengths: Tensor of shape (N,)
        Returns:
            Tensor of shape (T, N, C)
        """
        ...
        if input_lengths is not None:
            ...
            lengths = input_lengths.to(device).unsqueeze(1)  # (N, 1)
            src_key_padding_mask = time_idx >= lengths  # (N, T), True = pad
