...
class CNNBiLSTMCTCModule(pl.LightningModule):
    NUM_BANDS: ClassVar[int] = 2
    ELECTRODE_CHANNELS: ClassVar[int] = 16  # IMPORTANT: keep 16
    ...
    def __init__(
        ...
        # Model (matches TDSConvCTCModule style; only encoder differs)
        # inputs: (T, N, bands=2, electrode_channels=16, freq)
        self.model = nn.Sequential(
            SpectrogramNorm(channels=self.NUM_BANDS * self.ELECTRODE_CHANNELS),
            MultiBandRotationInvariantMLP(
    ...
    def _step(self, phase: str, batch: dict[str, torch.Tensor], *args, **kwargs) -> torch.Tensor:
        ...
        emissions = self.forward(inputs)  # (T, N, V)

        # No downsampling in this encoder -> same logic as TDSConvCTCModule
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff

        loss = self.ctc_loss(
            log_probs=emissions,               # (T, N, V)
            targets=targets.transpose(0, 1),   # (T, N) -> (N, T)
            input_lengths=emission_lengths,    # (N,)
            target_lengths=target_lengths,     # (N,)
        )
