...
class TDSConvCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        super().__init__()
        self.save_hyperparameters(ignore=["optimizer", "lr_scheduler", "decoder"])
...
class TransformerCTCModule(pl.LightningModule):
    ...
    def forward(
        ...
        x = self.band_mlp(x)  # (T, N, bands, mlp_features[-1])
        x = torch.flatten(x, start_dim=2)  # (T, N, num_features)
    ...
    def _step(
        ...
        # TransformerEncoder does not change temporal length.
...
class CNNTransformerCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        num_features = self.NUM_BANDS * mlp_features[-1]  # usually 2 * 384 = 768
        ...
        self.transformer = TransformerEncoder(
            num_features=conv_channels,  # IMPORTANT: transformer now works on CNN output dim
            num_layers=num_layers,
            nhead=nhead,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
        ...
        self.classifier = nn.Linear(conv_channels, charset().num_classes)
        self.log_softmax = nn.LogSoftmax(dim=-1)
    ...
    def forward(
        ...
        x = self.spec_norm(inputs)  # (T, N, bands, C, freq)
        x = self.band_mlp(x)  # (T, N, bands, mlp_features[-1])
        x = torch.flatten(x, start_dim=2)  # (T, N, 768)
        x = self.cnn(x)  # (T, N, conv_channels)
        x = self.transformer(x, input_lengths=input_lengths)  # (T, N, conv_channels)
        x = self.classifier(x)
        x = self.log_softmax(x)
        return x
    ...
    def _step(
        ...
        # CNN uses same padding, transformer keeps length => no temporal shrink
        emission_lengths = input_lengths
...
class CNNBiLSTMCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = nn.Sequential(
            ...
            nn.Flatten(start_dim=2),  # (T, N, 2, 384) -> (T, N, 768)
            ...
            nn.Linear(conv_channels, charset().num_classes),
            nn.LogSoftmax(dim=-1),
        )
    ...
    def _step(
        ...
        # CNNEncoder keeps T, BiLSTM keeps T
        T_diff = inputs.shape[0] - emissions.shape[0]
        emission_lengths = input_lengths - T_diff
